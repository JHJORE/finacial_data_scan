...
Usage:
    uv run python main.py                     # one-pass: search+read per company
    uv run python main.py --sample 50         # one-pass, random sample of 50
    uv run python main.py search              # search only (creates new run, saves progress)
    uv run python main.py read                # read + assemble (reuses latest search run)
...
async def process_company(company: Company, client, semaphore: asyncio.Semaphore):
    """One-pass pipeline for a single company: search then read immediately."""
...
async def cmd_run(args):
    """Full pipeline (one-pass): search + read per company, then assemble."""
